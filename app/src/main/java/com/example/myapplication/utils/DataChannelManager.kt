...
abstract class DataChannelManager<ViewState> {
    ...
    fun launchJob(
        ...
        println("DataChannelManager : Can Execute New StateEvent : $canExecute")
        if (canExecute) {
...
//        printLogD("DataChannelManager", "Cleared All State Messages")
    ...
    fun printAllStateMessages() {
        for (message in messageStack) {
            println("DataChannelManager : ${message.response.message}")
        }
    }
    ...
    private fun setupNewChannelScope(coroutineScope: CoroutineScope): CoroutineScope {
        ...
        println("DataChannelManager : Setting up New IO ChannelScope")
